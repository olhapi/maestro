...
export function IssueDetailPage() {
  ...
  const sessionMessage = (() => {
    ...
    if (
      execution.data.session_source === "persisted" &&
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-wrap gap-2">
                ...
                <Badge className="border-white/10 bg-white/5 text-white">
                  {toTitleCase(execution.data.retry_state)}
                </Badge>
                <Badge className="border-white/10 bg-white/5 text-white">
                  Attempt {execution.data.attempt_number || 0}
                </Badge>
                <Badge className="border-white/10 bg-white/5 text-white">
                  {toTitleCase(execution.data.phase || "implementation")}
                </Badge>
                {execution.data.failure_class ? (
                  <Badge className="border-rose-400/20 bg-rose-400/10 text-rose-100">
                    {toTitleCase(execution.data.failure_class)}
                  </Badge>
                ) : null}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-amber-50/90">
                        Maestro stopped retrying after{" "}
                        {execution.data.consecutive_failures ?? 0} interrupted
                        runs.
                        {execution.data.pause_reason
                          ? ` Last reason: ${execution.data.pause_reason}.`
                          : ""}
                      </p>
                      <p className="mt-2 text-amber-100/80">
                        Use Retry now after checking the workspace or runtime
                        conditions.
                      </p>
                    </div>
                  </div>
              ...
              <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge className="border-white/10 bg-white/5 text-white">
                    {toTitleCase(execution.data.session_source)}
                  </Badge>
                </div>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                  ...
                  <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                    Completed: {formatNumber(session?.turns_completed)}
                  </p>
                </div>
                <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">
                  ...
                  <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                    Updated:{" "}
                    {session ? formatDateTime(session.last_timestamp) : "n/a"}
                  </p>
                </div>
                <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-[var(--muted-foreground)]">
                  ...
                  <p className="mt-2 font-display text-3xl text-white">
                    {formatNumber(issue.data.total_tokens_spent)}
                  </p>
                  <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                    Lifetime tokens across all runs
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                ...
                <div className="mt-4 space-y-3">
                  {sessionHistory.length === 0 ? (
                    <p className="text-sm text-[var(--muted-foreground)]">
                      No session history captured for this issue yet.
                    </p>
                  ) : (
                    sessionHistory.map((event, index) => (
                      <div
                        key={`${event.type}-${event.turn_id || index}`}
              ...
              <div className="rounded-[1.5rem] border border-white/8 bg-black/20 p-4">
                ...
                <div className="mt-4 space-y-3">
                  {runtimeEvents.length === 0 ? (
                    <p className="text-sm text-[var(--muted-foreground)]">
                      No persisted runtime events for this issue yet.
                    </p>
                  ) : (
