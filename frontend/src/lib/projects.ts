import type { EpicSummary, Project, ProjectSummary, StateBucket } from '@/lib/types'
...
export function projectDispatchLabel(project: Pick<Project, 'orchestration_ready' | 'dispatch_ready' | 'dispatch_error'>) {
...
export function projectDispatchBadgeClass(project: Pick<Project, 'orchestration_ready' | 'dispatch_ready' | 'dispatch_error'>) {
  if (isProjectDispatchReady(project)) {
...
function fallbackCounts(summary: Pick<ProjectSummary | EpicSummary, 'counts'>) {
  return {
    ...
    active: summary.counts.ready + summary.counts.in_progress + summary.counts.in_review,
...
export function summaryActiveCount(summary: ProjectSummary | EpicSummary) {
  ...
  return summary.active_count ?? (bucketTotals.active || fallbackCounts(summary).active)
}

export function summaryTotalCount(summary: ProjectSummary | EpicSummary) {
  const bucketTotals = countBuckets(summary.state_buckets)
  return summary.total_count ?? (bucketTotals.total || fallbackCounts(summary).total)
}

export function summaryTerminalCount(summary: ProjectSummary | EpicSummary) {
  const bucketTotals = countBuckets(summary.state_buckets)
  return summary.terminal_count ?? (bucketTotals.terminal || fallbackCounts(summary).terminal)
}

export function summaryDoneCount(summary: ProjectSummary | EpicSummary) {
  return summary.state_buckets?.find((bucket) => bucket.state === 'done')?.count ?? fallbackCounts(summary).done
}
