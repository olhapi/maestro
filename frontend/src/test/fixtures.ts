...
export function makeBootstrapResponse(overrides: Partial<BootstrapResponse> = {}): BootstrapResponse {
  ...
  return {
    ...
    overview: {
        ...
        running: [
          {
            ...
            tokens: { input_tokens: 5, output_tokens: 8, total_tokens: 13, seconds_running: 60 },
          },
        ],
        retrying: [
          {
      ...
      epic_count: 1,
      issue_count: 1,
      series: [{ bucket: '12:00', runs_started: 1, runs_completed: 1, runs_failed: 0, retries: 1, tokens: 13 }],
      recent_events: [{ seq: 1, kind: 'run_started', identifier: issue.identifier, ts: '2026-03-09T12:00:00Z' }],
    },
    projects: [
      {
        id: 'project-1',
        ...
        counts: { backlog: 0, ready: 1, in_progress: 1, in_review: 0, done: 2, cancelled: 0 },
      },
    ],
    epics: [
      {
