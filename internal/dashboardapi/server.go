...
func (s *Server) issueExecutionPayload(issue *kanban.Issue) (map[string]interface{}, error) {
	events, err := s.store.ListIssueRuntimeEvents(issue.ID, 50)
	if err != nil {
		return nil, err
	}
	...
	if session, ok := findLiveSession(s.provider.LiveSessions(), issue.Identifier); ok {
		liveSession = &session
	}

	persistedSession, err := s.store.GetIssueExecutionSession(issue.ID)
	if err != nil && err != sql.ErrNoRows {
		return nil, err
	}
