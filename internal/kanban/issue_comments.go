...
func (s *Store) ListIssueCommentsPage(issueID string, limit, offset int) ([]IssueComment, int, error) {
	if strings.TrimSpace(issueID) == "" {
		return nil, 0, validationErrorf("issue_id is required")
	}
	if _, err := s.GetIssue(issueID); err != nil {
		if err == sql.ErrNoRows {
			return nil, 0, notFoundError("issue", issueID)
		}
		return nil, 0, err
	}
	...
	if err := s.db.QueryRow(`
		SELECT COUNT(*)
		FROM issue_comments
		WHERE issue_id = ? AND parent_comment_id = ''`,
		issueID,
	).Scan(&total); err != nil {
		return nil, 0, err
	}
	...
	if len(rootIDs) == 0 {
		return []IssueComment{}, total, nil
	}

	placeholders := strings.TrimSuffix(strings.Repeat("?,", len(rootIDs)), ",")
	args := make([]interface{}, 0, len(rootIDs)+1)
	...
	query := `
		...
		tree AS (
			...
			FROM issue_comments
			WHERE issue_id = ? AND id IN (SELECT id FROM selected_roots)
			UNION ALL
			SELECT child.id, child.issue_id, child.parent_comment_id, child.body, child.author_json, child.provider_kind, child.provider_comment_ref, child.deleted_at, child.created_at, child.updated_at
			FROM issue_comments child
			INNER JOIN tree parent ON child.parent_comment_id = parent.id AND child.issue_id = parent.issue_id
		)
		SELECT id, issue_id, parent_comment_id, body, author_json, provider_kind, provider_comment_ref, deleted_at, created_at, updated_at
		FROM tree
		ORDER BY created_at ASC, id ASC`
	...
	return nestIssueComments(comments), total, nil
}
