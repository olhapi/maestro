...
func TestRunAgentAppServerPrependsAndClearsPendingPlanRevision(t *testing.T) {
	traceFile := filepath.Join(t.TempDir(), "trace.log")
	t.Setenv("TRACE_FILE", traceFile)
	...
	if len(turnStarts) != 1 {
		t.Fatalf("expected one turn/start payload, got %#v", turnStarts)
	}
	params, _ := turnStarts[0]["params"].(map[string]interface{})
	input, _ := params["input"].([]interface{})
	if len(input) == 0 {
		t.Fatalf("expected prompt input, got %#v", params["input"])
	}
	firstInput, _ := input[0].(map[string]interface{})
	prompt, _ := firstInput["text"].(string)
	if !strings.HasPrefix(strings.TrimSpace(prompt), "Plan revision note:") {
		t.Fatalf("expected plan revision note to be prepended, got %q", prompt)
	}
