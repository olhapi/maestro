...
Use `--desc` for durable project guidance, not just a label. Maestro includes `project.description` in the default implementation, review, and done prompts for every issue in that project.
...
`--log-level` defaults to `warn`. Use `info` when you want lifecycle and status transition logs, and `debug` when you need raw runtime stream output and session churn in the structured logs.
