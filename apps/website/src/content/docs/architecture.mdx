...
Before Docker starts, the npm launcher does a single preflight pass over the
database and workflow files so it can mount repo and worktree roots, including
custom `workspace.root` values, without relying on a separate SQLite CLI.
