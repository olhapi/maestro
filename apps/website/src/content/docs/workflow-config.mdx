...
description: Tune how much work Maestro can offload, how aggressively it retries, and how Codex is launched through WORKFLOW.md.
section: concepts
order: 2
...
- `codex.initial_collaboration_mode`: controls the startup mode for fresh `app_server` threads. Use `default` for unattended execution-first runs, or `plan` when you explicitly want a plan-gated startup. This does not affect `stdio` or resumed threads.
...
- `initial_collaboration_mode: default` starts a fresh unattended `app_server` thread in execution mode. Use `plan` only for explicitly plan-gated runs.
...
1. `Workspace root`
2. `Codex command`
3. `Agent mode (app_server|stdio)`
4. `Dispatch mode (parallel|per_project_serial)`
5. `Max concurrent agents`
6. `Max turns`
7. `Max automatic retries`
8. If `agent.mode` is `app_server`, `Approval policy (never|on-request|on-failure|untrusted)`
9. If `agent.mode` is `app_server`, `Initial collaboration mode (default|plan)`

The additional dispatch, retry, approval, and collaboration choices are interactive-only. `--defaults` remains the stable non-interactive scaffold, and the existing flags still override only `workspace.root`, `codex.command`, and `agent.mode`.

Use flags when you want to skip prompts but still override part of the scaffold:

<DocsCommandField
  command={`maestro workflow init . --defaults --workspace-root ./tmp/workspaces --codex-command "codex app-server" --agent-mode app_server`}
/>
