...
The repository CI workflow will use `TURBO_TEAM` and `TURBO_TOKEN` automatically when those are configured in GitHub.
...
The hooks stay targeted. Frontend changes run frontend lint and tests, website changes run Astro checks and website tests, workspace and hook changes run the full `pnpm verify` suite, and `pre-push` runs that same full verifier before the push proceeds.
