...
description: Use the embedded dashboard to see whether Codex should keep running or whether a developer needs to step back in.
section: concepts
order: 1
...
Use the overview page before you open a specific issue. It tells you whether Codex is healthy enough to leave alone or whether it needs attention:
...
Issue pages are intentionally dense because this is the main re-entry surface. They keep the execution transcript, live state, blockers, branch and PR state, token usage, image attachments, and run controls close together so you do not have to context-switch during supervision.
...
The sessions surface is for active Codex orchestration periods when multiple workspaces are moving at once. Use it to read live session identifiers, last activity, and retry context without opening each issue individually.
