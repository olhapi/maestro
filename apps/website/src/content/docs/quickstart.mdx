...
If `maestro` is already on your `PATH`, save it through the MCP flow your coding agent supports. Use the direct commands for Codex or Claude Code when they fit, or the generic server entry for other MCP-capable agents.
...
Before Docker starts, the launcher does a single preflight pass over the
database and workflow files. It resolves `workspace.root` for every discovered
workflow and mounts the repo and worktree directories up front, so custom
workspace roots keep working during startup.
